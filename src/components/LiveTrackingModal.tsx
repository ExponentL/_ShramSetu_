...
export const LiveTrackingModal: React.FC = () => {
  ...
  const progressionStatus = useMemo(() => {
    switch (activeBooking?.status) {
      case 'Accepted':
      case 'Requested':
        return { index: 0, label: 'Worker assigned' };
      case 'Worker On The Way':
        return { index: 1, label: 'Worker on the way' };
      case 'Arrived':
        return { index: 2, label: 'Worker arrived nearby' };
      case 'Diagnosing':
      case 'Diagnosis Completed':
      case 'Work Started':
        return { index: 3, label: 'Job started' };
      case 'Work Completed':
      case 'Payment Completed':
        return { index: 4, label: 'Job completed' };
      default:
        return { index: 1, label: 'Worker on the way' };
    }
    ...
    <div className="max-w-7xl mx-auto space-y-6 pb-12">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-4">
          {/* Mobile Worker Tracking Header Card */}
          <div className="lg:hidden bg-white rounded-2xl border border-[#E2DFD8] p-4 shadow-sm flex items-center justify-between gap-3">
            <div className="flex items-center gap-3">
              <div className="w-14 h-14 rounded-xl overflow-hidden shrink-0 border border-[#E2DFD8] bg-slate-100">
                <img
                  src={activeBooking.workerPhoto || 'https://images.unsplash.com/photo-1621905251189-08b45d6a269e?w=400&auto=format&fit=crop&q=80'}
                  alt={activeBooking.workerName}
                  className="w-full h-full object-cover"
                />
              </div>
              <div>
                <div className="font-black text-slate-900 text-sm leading-tight">{activeBooking.workerName}</div>
                <div className="text-xs font-semibold text-slate-600">{activeBooking.serviceCategory === 'Electrical' ? 'Electrician' : activeBooking.serviceCategory}</div>
                <div className="inline-flex items-center gap-1 text-[10px] font-bold text-[#167A5B] mt-0.5">
                  <CheckCircle className="w-3 h-3 text-[#167A5B]" />
                  <span>✓ Verified Worker</span>
                </div>
                <div className="text-[11px] font-bold text-slate-700 mt-1 flex items-center gap-2">
                  <span className="text-[#167A5B]">On the way</span>
                  <span className="text-slate-300">•</span>
                  <span>1.4 km away</span>
                </div>
                <div className="text-[10px] text-slate-400">Location updated 2 min ago</div>
              </div>
            </div>

            <div className="shrink-0 flex flex-col gap-1.5">
              <a
                href={`tel:${activeBooking.workerPhone || '+919818099887'}`}
                className="min-h-[44px] px-3.5 py-2 rounded-xl bg-[#17324D] text-white text-xs font-bold flex items-center justify-center gap-1.5 shadow-xs cursor-pointer btn-tactile"
              >
              ...
              <button
                type="button"
                onClick={() => alert('Cooperative Support Helpline: 1800-419-2667 (Toll-Free, 24x7)')}
                className="min-h-[44px] px-2 py-1.5 rounded-xl border border-slate-300 text-slate-700 text-[11px] font-bold flex items-center justify-center gap-1 bg-white hover:bg-slate-50 cursor-pointer"
              >
          ...
          <div className="bg-white rounded-3xl border border-slate-200 shadow-sm p-4 sm:p-5 space-y-4">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
              ...
              <div className="w-full p-2.5 bg-[#FAF8F5] rounded-xl border border-[#E2DFD8] flex items-center justify-between gap-1 overflow-x-auto text-[11px]">
                {[
                  { key: 0, label: 'Worker assigned' },
                  { key: 1, label: 'Worker on the way' },
                  { key: 2, label: 'Worker arrived nearby' },
                  { key: 3, label: 'Job started' },
                  { key: 4, label: 'Job completed' },
                ].map((st, i) => {
                  const isCurrent = progressionStatus.index === st.key;
                  const isPast = progressionStatus.index > st.key;
                  return (
                    <div key={st.key} className="flex items-center gap-1.5 shrink-0">
                      <span
                        className={`w-2 h-2 rounded-full ${
                          isCurrent
                            ? 'bg-[#17324D] ring-2 ring-slate-300'
                            : isPast
                            ? 'bg-[#167A5B]'
                            : 'bg-slate-300'
                        }`}
                      ...
                      <span
                        className={`${
                          isCurrent
                            ? 'font-black text-[#17324D]'
                            : isPast
                            ? 'font-bold text-[#167A5B]'
                            : 'font-medium text-slate-400'
                        }`}
                      >
                        {st.label}
                      </span>
                      {i < 4 && <span className="text-slate-300 mx-1">&rarr;</span>}
                    </div>
                  );
                })}
              </div>
            ...
            {/* Mobile Booking Timeline Below Map */}
            <div className="lg:hidden p-4 rounded-2xl bg-white border border-[#E2DFD8] space-y-3">
              <div className="text-xs font-bold uppercase tracking-wider text-slate-500 border-b border-slate-100 pb-2 flex items-center justify-between">
                ...
                <span className="text-[10px] text-[#167A5B] font-bold">Real-time status</span>
              </div>
              <div className="space-y-2 text-xs">
                {[
                  { label: 'Confirmed', done: true },
                  { label: 'Assigned', done: true },
                  { label: 'Travelling', active: isWorkerOnWay && !isArrived, done: isArrived || isWorkStarted || isWorkCompleted },
                  { label: 'Arrived', active: isArrived, done: isWorkStarted || isWorkCompleted },
                  { label: 'Job Started', active: isWorkStarted, done: isWorkCompleted },
                  { label: 'Completed', done: isWorkCompleted },
                ].map((item) => (
                    ...
                    <span
                      className={`w-5 h-5 rounded-full flex items-center justify-center text-[10px] font-black shrink-0 ${
                        item.done
                          ...
                          : item.active
                          ? 'bg-[#17324D] text-white ring-2 ring-slate-300 animate-pulse'
        ...
        <div className="space-y-4">
          ...
          <div className="bg-white rounded-3xl border border-slate-200 shadow-sm p-5 space-y-4">
            <div className="flex items-start gap-3.5">
              ...
              <div className="min-w-0 flex-1">
                <div className="flex items-center gap-2 flex-wrap">
                  <h3 className="font-black text-slate-900 text-base truncate">{activeBooking.workerName || 'Rajesh Kumar'}</h3>
                  <span className="inline-flex items-center gap-1 text-[10px] font-bold text-[#167A5B] bg-[#EDF7F2] px-2 py-0.5 rounded-md border border-[#C6E7D8] shrink-0">
                    <span>✓</span> Verified Worker
                  </span>
                </div>

                <div className="text-xs font-bold text-[#17324D] mt-0.5">
                  {t.categories[activeBooking.serviceCategory as ServiceCategory] || activeBooking.serviceCategory || 'Electrician'}
                </div>

                <div className="text-xs text-slate-600 mt-2 space-y-1">
                  <div className="flex items-center gap-1.5 font-medium">
                    <MapPin className="w-3.5 h-3.5 text-[#167A5B]" />
                    <span>{isArrived ? 'Arrived at destination' : `${remainingDistance || '1.4'} km away`}</span>
                  </div>
                ...
                <p className="text-[11px] text-slate-500 truncate mt-2 pt-2 border-t border-[#EBE8E1] font-medium">
            ...
            {/* Tracking Timeline with live indicator on active step */}
            <div className="pt-3 border-t border-slate-100 space-y-3">
              <div className="flex items-center justify-between text-[11px] uppercase font-bold text-slate-500">
                ...
                <span className="text-[#167A5B] font-bold">● Live Tracking</span>
              </div>

              <div className="space-y-3">
                {[
                  { id: 'step-confirmed', title: 'Booking confirmed', state: 'done' },
                  { id: 'step-assigned', title: 'Worker assigned', state: 'done' },
                  {
                    id: 'step-travelling',
                    title: 'Worker travelling',
                    state: progressionStatus.index === 1 ? 'active' : progressionStatus.index > 1 ? 'done' : 'pending',
                  },
                  {
                    id: 'step-arrived',
                    title: 'Worker arrived',
                    state: progressionStatus.index === 2 ? 'active' : progressionStatus.index > 2 ? 'done' : 'pending',
                  },
                  {
                    id: 'step-started',
                    title: 'Job started',
                    state: progressionStatus.index === 3 ? 'active' : progressionStatus.index > 3 ? 'done' : 'pending',
                  },
                  {
                    id: 'step-completed',
                    title: 'Job completed',
                    state: progressionStatus.index >= 4 ? 'done' : 'pending',
                  },
                ].map((item, idx) => (
                    ...
                    <div className="flex flex-col items-center">
                      <div
                        className={`w-5 h-5 rounded-full flex items-center justify-center text-[10px] font-black transition-all ${
                          item.state === 'done'
                            ? 'bg-[#167A5B] text-white shadow-2xs'
                            : item.state === 'active'
                            ? 'bg-[#17324D] text-white ring-2 ring-slate-300'
                      ...
                      {idx < 5 && (
                        <div
                          className={`w-0.5 h-4 my-0.5 ${
                            item.state === 'done' ? 'bg-[#167A5B]' : 'bg-slate-200'
                          }`}
                        />
                      )}
                    </div>
